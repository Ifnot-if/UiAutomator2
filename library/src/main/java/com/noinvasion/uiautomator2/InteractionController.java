...
class InteractionController {
    ...
    public InteractionController(UiAutomation uiAutomation) {
    ...
    private AccessibilityEvent runAndWaitForEvents(Runnable command,
        ...
        try {
            ...
            LogUtil.e("runAndwaitForEvents timed out waiting for events");
            return null;
        } catch (Exception e) {
            LogUtil.e("exception from executeCommandAndWaitForAccessibilityEvent", e);
            return null;
        }
    }
    ...
    public boolean scrollSwipe(final int downX, final int downY, final int upX, final int upY,
                               final int steps) {
        LogUtil.d("scrollSwipe (" + downX + ", " + downY + ", " + upX + ", "
                + upY + ", " + steps + ")");
        ...
        if (event.getFromIndex() != -1 && event.getToIndex() != -1 && event.getItemCount() != -1) {
            ...
            LogUtil.d("scrollSwipe reached scroll end: " + foundEnd);
            ...
            if (downX == upX) {
                ...
                LogUtil.d("Vertical scrollSwipe reached scroll end: " + foundEnd);
            } else if (downY == upY) {
                // Horizontal
                foundEnd = event.getScrollX() == 0 ||
                        event.getScrollX() == event.getMaxScrollX();
                LogUtil.d("Horizontal scrollSwipe reached scroll end: " + foundEnd);
            }
        }
        recycleAccessibilityEvents(events);
        return !foundEnd;
    ...
    public boolean sendKey(int keyCode, int metaState) {
        LogUtil.d("sendKey (" + keyCode + ", " + metaState + ")");
